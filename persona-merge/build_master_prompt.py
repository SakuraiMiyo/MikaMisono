...
def grab(secs: list[dict], title: str) -> str:
    ...
    want = norm(title)
    # 1) 完全精确（最优先，避免 "禁止事项" 抢到 "禁止事项 · 平台流程"）
    for s in secs:
        if s["title"] == title:
            return s["raw"]
    # 2) 去记号后完全相同
    for s in secs:
        if norm(s["title"]) == want:
            return s["raw"]
    # 3) 去记号后以它开头。**取最短的那一条**——前缀匹配天然有歧义，
    #    最短标题就是最贴近原意的那个（避免 "世界观" 抢到别的长标题）。
    cand = [s for s in secs if norm(s["title"]).startswith(want)]
    if cand:
        return min(cand, key=lambda s: len(norm(s["title"])))["raw"]
...
def main() -> None:
    ...
    part2 = [
        ...
        ("2.0a　⚠️ 害羞可以，但说完要往前走（2026-09-26）",
         grab(QQ, "⚠️ 害羞可以，但**说完要往前走**（2026-09-26）")),
    ...
    part3 = [
        ...
        ("3.6　你不知道的事（知识边界与纠错）", grab(DSH, "十、你不知道的事")),
    ]
