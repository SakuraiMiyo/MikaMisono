    ...
    if not config.kb_agentic_mode:
        ...
        try:
            ...
            if not kb_result:
                return
            # ── 2026-09-24 调整注入位置 ────────────────────────────────────────
            # 原实现是塞进 extra_user_content_parts，那会被拼进**当前 user 消息**，
            # 也就是模型生成前看到的**最后一段**。知识库内容是第三人称的分析文字，
            # 放在这个位置会强烈污染输出语域——实测表现为角色开始写"资料综述腔"、
            # 长段落、条目体，即使在 system prompt 里反复强调也没用。
            # 改成写进 system_prompt（index 0，离生成点最远），让它退成"背景资料"。
            # ── 2026-09-26 改动 ────────────────────────────────────────────
            ...
            kb_block = (
                ...
                "**用你自己的口气讲出来**——像跟老师聊起一件旧事那样"
                "（「诶，我记得那天……」「哦对，就是上回——」），"
                "不要照着念，更不要因此把回复写成条目或报告。\n"
                "名字、日期、数字、剧情顺序要准；说法要是你的说法。\n\n"
                f"{kb_result}\n"
            )
            req.system_prompt = (req.system_prompt or "") + kb_block
