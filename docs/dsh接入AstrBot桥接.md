...
> ## ⚠️ 当前状态（2026-09-24 核对）：**链路已断开，桥接在空转**
>
> - `C:\SoftWare\Astrbot\dsh-bridge\dsh_bridge.py`（127.0.0.1:8808）**进程在跑、开机自启正常**，
>   `curl http://127.0.0.1:8808/v1/models` 也能返回。
> - 但 AstrBot 侧 **`provider` 列表里没有 `dsh/dsh`**，**`mcp_server.json` 里也没有 dsh** ——
>   也就是说 QQ 机器人现在由 `deepseek/deepseek-flash` **直连**回答，桥接没有被调用。
> - 本手册的配置步骤（第四节 4.6、MCP 部分）是**恢复接线时**要照做的，不是当前生效状态。
> - 恢复接线会改动线上 AstrBot 的模型选择，**请在老师在场时单独进行**。
>
> 系统全貌见 `docs\未花系统总览.md`。
